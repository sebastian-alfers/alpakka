 ...
 */
@InternalApi
private[amqp] final class AmqpRpcFlowStage(classSettings: AmqpWriteSettings, bufferSize: Int, responsesPerMessage: Int = 1)
  ...
  override def createLogicAndMaterializedValue(inheritedAttributes: Attributes): (GraphStageLogic, Future[String]) = {
    ...
    (new GraphStageLogic(shape) with AmqpConnectorLogic {
      ...
      override val settings = stage.classSettings
      private val exchange = classSettings.exchange.getOrElse("")
      private val routingKey = classSettings.routingKey.getOrElse("")
